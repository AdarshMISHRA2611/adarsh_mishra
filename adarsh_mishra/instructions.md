...
- **Game page:** Back button, game title, best score display ("Your best: N" or "Your best: none"), iframe of minimum height 400px with sandbox attributes, score input + Save Score button, Save Bookmark button, Add to Favourites toggle. If the iframe fails to load, show "Game could not be loaded. Try another one."
...
1. [ ] Home page loads with at least 9 game cards visible within the viewport
2. [ ] Home page game grid renders in 3 columns at viewport width 1024px or wider
3. [ ] Home page game grid renders in 2 columns at viewport width 768px to 1023px
4. [ ] Every game card shows game name, category icon, rating badge with number, and info button
5. [ ] Search bar filters the grid to show cards whose name or genre matches the input, updating instantly
6. [ ] Left sidebar contains at least 6 clickable genre category icons
7. [ ] Clicking a sidebar category filters the game grid to show games of that genre
8. [ ] Clicking any game card navigates to the Game page for that game
9. [ ] Game page contains an iframe with src set to the selected game's URL
10. [ ] Game page has a back button that returns to the Home page
11. [ ] Save Score button stores an entry to cabby_scores with game, score, and date
12. [ ] Add to Favourites button toggles the game ID in cabby_favourites on each click
13. [ ] Leaderboard page has a tab switcher between All Scores and Your Best that updates instantly
14. [ ] All Scores rows are sorted by Score descending with the highest score at Rank 1
15. [ ] All Scores table renders one row for every entry in cabby_scores
16. [ ] Clear All Scores button shows a confirmation prompt before deleting cabby_scores
17. [ ] Profile page displays the saved display name from cabby_username
18. [ ] Edit Name action saves the new name to cabby_username and survives reload
19. [ ] Profile favourites list shows each favourited game with its own working remove button
20. [ ] Reset Profile button shows a confirmation prompt before clearing all CABBY localStorage keys
21. [ ] CABBY logo image from logo.png appears in the top-left corner of the nav on every page
22. [ ] Colour #1D9E75 is used for Save Score button, Log in button, and rating badge backgrounds
23. [ ] Colour #F5C842 is used for the CABBY brand text and active sidebar icon
24. [ ] Colour #E24B4A is used for Clear All Scores, Reset Profile, and notification badge
25. [ ] Every localStorage key starts with the cabby_ prefix
26. [ ] cabby_scores entries are objects with exactly the fields game, score, and date
27. [ ] Website is accessible via a public URL deployed on a free static host
28. [ ] Source code is in a public GitHub repository with a README.md at the root
29. [ ] Home page displays a Today's Missions section with exactly 3 mission cards above the game grid
30. [ ] Same calendar date shows the same 3 missions to every user, drawn from the pool of 10 templates
31. [ ] Completing all 3 missions in a day increments cabby_streak.current by exactly 1
32. [ ] cabby_streak.best is updated to max(best, current) whenever current increases
33. [ ] Navigation bar displays a flame icon and the current streak number on every page
34. [ ] Save Bookmark button opens an inline text input that accepts up to 80 characters with a Save action
35. [ ] Submitting the Save Bookmark form stores an object with id, gameId, gameName, note, date, lastScore to cabby_bookmarks
36. [ ] Profile page Bookmarks section lists every saved bookmark with game name, note, and date
37. [ ] Each bookmark item has a delete button that removes that entry from cabby_bookmarks
38. [ ] Clicking the game name of a bookmark navigates to the Game page for that bookmarked game
39. [ ] Submitting a higher score than the existing value updates cabby_best_scores for that game ID
40. [ ] Game page displays the user's current best score above the iframe as "Your best: N"

---

## Evaluation Criteria

The project is scored against the 40 items in the Acceptance Checklist above. Each item is Pass or Fail. The build is accepted when 32 or more items pass (80 percent threshold). Visual fidelity to `reference_image.png`, correctness of localStorage behaviour, deterministic missions, accurate streak logic, working bookmarks, and accurate best-score tracking are the highest-weighted areas of judgement.
